...
files = glob.glob(os.path.join(args.gt_renders_dir, "*_rgb.png"))
numfiles = len(files)
...
with torch.no_grad():
    for i in tqdm.tqdm(range(numfiles)):
        pred_rgb = np.array(Image.open(pred_rgb_files_format.format(i+1))) / 255.
        ...
        # Depth
        pred_depth = np.load(pred_depth_files_format.format(i+1)).squeeze()
...
results_file = os.path.join(args.exp_renders_dir, "results.json")
